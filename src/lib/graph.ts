...
export async function getUserMeetings(
  ...
  try {
    ...
    // Return empty array instead of throwing - let the caller handle missing data gracefully
    return [];
  }
}
...
export interface EmailMessage {
  ...
  importance: string;
  isRead: boolean;
  conversationId?: string; // Added for email chain handling
}
...
export interface RelatedEmailMessage extends EmailMessage {
  ...
  matchReason?: string;
  fullBody?: string;
  isPartOfChain?: boolean; // Added to indicate if email is part of a thread
  chainEmailCount?: number; // Number of emails in the conversation thread
}
...
/**
 * Deduplicates emails by conversation, keeping only the most recent email per conversation
 * This is useful for meeting prep to avoid processing entire email threads
 * @param emails - Array of email messages
 * @returns Deduplicated array with most recent email per conversation
 */
export function deduplicateEmailsByConversation(
  emails: EmailMessage[]
): EmailMessage[] {
  if (emails.length === 0) {
    return [];
  ...
  const conversationMap = new Map<string, EmailMessage[]>();
  
  for (const email of emails) {
    const convId = email.conversationId || email.id; // Fall back to email ID if no conversationId
  ...
  // For each conversation, keep only the most recent email
  const deduplicated: EmailMessage[] = [];
  
  for (const [conversationId, conversationEmails] of conversationMap.entries()) {
    // Sort by receivedDateTime descending (most recent first)
    conversationEmails.sort((a, b) => 
      new Date(b.receivedDateTime).getTime() - new Date(a.receivedDateTime).getTime()
    );
    
    const mostRecent = conversationEmails[0];
    
    // Add metadata about the conversation chain
    if (conversationEmails.length > 1) {
      (mostRecent as RelatedEmailMessage).isPartOfChain = true;
      (mostRecent as RelatedEmailMessage).chainEmailCount = conversationEmails.length;
    }
    
    deduplicated.push(mostRecent);
  }

  // Sort final results by receivedDateTime descending
  deduplicated.sort((a, b) => 
    new Date(b.receivedDateTime).getTime() - new Date(a.receivedDateTime).getTime()
  );

  console.log(`📧 Deduplicated ${emails.length} emails → ${deduplicated.length} (removed ${emails.length - deduplicated.length} chain replies)`);
...
export async function getTeams(
  ...
  try {
    ...
    // Return empty array instead of throwing - Teams access is optional
    return [];
  }
}
...
export async function getTeamChannels(
  ...
  try {
    ...
    // Return empty array instead of throwing - channel access might be restricted
    return [];
  }
}
...
export async function getChannelMessages(
  ...
  try {
    ...
    if (options?.since) {
      query = query.filter(`createdDateTime ge ${options.since}`);
    }
...
export async function getRecentFiles(
  ...
  try {
    ...
    // Return empty array instead of throwing - OneDrive access is optional
    return [];
  }
}
...
export async function getRecentEmailsWithPeople(
  ...
  try {
    ...
    const response = await client
      ...
      .select("id,subject,from,toRecipients,receivedDateTime,bodyPreview,importance,isRead,hasAttachments,conversationId")
      .orderby("receivedDateTime desc")
      .top(20)
      .get();
